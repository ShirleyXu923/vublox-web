...
import './TrendingEventsTab.scss';

const placeholder = Array(6).fill(null);

function TrendingEventsTab() {
  const i18n = useTranslation('trending');
  const categories = useSelector((state: IRootState) => state.Category.filters ?? []);
  const [ events, setEvents ] = useState<any>({
    trending: [],
    latest: [],
  });
  const interests = useSelector((state: IRootState) => (state.Auth.user as any).interests ?? []);
  ...
  const loadFilteredData = async (filter = 'trending') => {
    try {
      const cat = filters.length === 0 ? categories : filters;
  ...
  return (
    <div className={`trending-events ${isSmScreen ? 'mt-4' : 'mt-5'}`}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        {!isSmScreen && <h1>{i18n.label.trending}</h1>}
              ...
              <div className="d-flex">
                {LocaleService.parseTranslation(i18n.label.empty2, {
                  selectCategory: (
                    <Button
                      color="link"
                      className="p-0 m-0 fw-bold mx-1"
                      onClick={() => eventFilter?.current?.open?.()}
                    >
                      {i18n.label.selectCategory}
                    </Button>
                  ...
                  follow: (
                    <Button
                      color="link"
                      className="p-0 m-0 d-inline fw-bold mx-1"
                      onClick={() => setShowFollowSelection(true)}
                    >
                      {i18n.label.follow}
                    </Button>
...
export default TrendingEventsTab;
