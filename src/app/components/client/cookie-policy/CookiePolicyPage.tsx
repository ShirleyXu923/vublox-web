...
const links = {
  link1: {
    title: 'AllAboutCookies.org',
    link: 'allAboutCookies.org',
  },
  link2: {
    title: 'Your online choices',
    link: 'youronlinechoices.com',
  },
};

function CookiePolicyPage() {
  const i18n = useTranslation('cookiePolicy');
  const [ effectiveDate ] = useState(moment('08-15-2024', 'MM-DD-YYYY').format('DD MMMM YYYY'));

  const formatDescription = (description = '') => (description.includes('{email}') || description.includes('{link')
    ? LocaleService.parseTranslation(description, {
      email: (
        <a href={`mailto:${app.contactEmail}`}>{app.contactEmail}</a>
      ),
      link1: (
        <a href={links.link1.link}>{links.link1.title}</a>
      ),
      link2: (
        <a href={links.link2.link}>{links.link2.title}</a>
      ),
    }) : description);
  ...
  return (
    <Container fluid className="cookie-policy-page">
