...
import { getFollowersRequest } from '@reducers/follow/FollowAction';
...
import FollowerPlaceholder from './components/follower-placeholder/FollowerPlaceholder';
import './FollowersModal.scss';

function FollowersModal({ show, toggle, id }: {
  show: boolean;
  toggle: () => void;
  id: string | undefined;
}) {
  ...
  const loadData = async (refresh = false, query = '') => {
    ...
    try {
      const { data } = await dispatch(getFollowersRequest(params)).$promise;
    ...
    >
      <ModalHeader toggle={toggle}>
        <h1 className="s1 mb-0">
          {startCase(i18n.label.followers)}
        </h1>
      </ModalHeader>
      <ModalBody id="followers-scrollable">
        <SearchBar
          onSearch={onSearch}
          loading={searching}
          placeholder={i18n.placeholder.searchFollowing}
          inputProps={{ value: keyword }}
        />
        ...
        <InfiniteScroll
          ...
          scrollableTarget="followers-scrollable"
        >
          {items.map((item: any) => (
