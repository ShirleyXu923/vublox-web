...
function TimelineCard({ data, onClick }: { data: any; onClick?: () => void }) {
  ...
  return (
    <div className="group-card-item timeline">
      <Link className="stretched-link" to={`/timelines/${data?.id}`} target="_blank" onClick={onClick} />
      {getThumbnail()}
      <div className={classNames({
        'd-flex align-items-center': true,
        'mt-3': (!data?.description && data?.cover_image) || (!data?.description && !data?.cover_image) || data?.cover_image,
      })}
      >
      ...
      <div className="d-flex align-items-center gap-2 mt-auto pt-3 relative">
