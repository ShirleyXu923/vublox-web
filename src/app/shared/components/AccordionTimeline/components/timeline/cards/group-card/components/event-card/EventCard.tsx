...
function EventCard({ data, onClick }: { data: any; onClick?: () => void }) {
  ...
  const getThumbnail = () => {
    ...
    if (!data?.banner_url && data?.description && !data?.metadata) {
      return null;
    }
  ...
  return (
    <div className="group-card-item">
      ...
      <div className={classNames({
        ...
        'mt-3': (!data?.description && data?.banner_url) || (!data?.description && !data?.banner_url) || data?.banner_url || data.metadata,
      })}
      >
        ...
        <div className="d-flex align-items-center text-truncate wrap">
          ...
          <FollowLink followId={data.id} followType="Event" />
        </div>
      </div>
      ...
      <div className="content mt-auto pt-3 relative text-truncate">
        <Link className="stretched-link" to={`/events/${data.id}`} target="_blank" onClick={onClick} />
        <div className="d-flex align-items-center">
          ...
          <div className="me-1">
                ...
                {(data.coCreators > 0 || data.owner) && (
                  <div className="caption1 text-truncate">
                    ...
                    {data.coCreators?.length > 0 && (
                      <>
                        {', '}
                        {data.coCreators.map((c: any) => (
                          <NavLink key={c.id} to={c.creator?.link} className="link">
                            {c.creator?.name}
                          </NavLink>
            ...
            <div className="secondary-text caption1 text-truncate creator d-flex align-items-center">
